...
function parseManifestBlock(frontmatter: string): {
  ...
  const references: Array<{ file: string; loadWhen?: { task?: string; chapterIndexMax?: number } }> = []
  let currentReference: { file: string; loadWhen?: { task?: string; chapterIndexMax?: number } } | null = null
  let inReferenceLoadWhen = false
  ...
  const flushReference = (): void => {
    if (currentReference?.file) references.push(currentReference)
    currentReference = null
    inReferenceLoadWhen = false
  }

  for (const line of lines) {
    if (line.match(/^[A-Za-z]/) && !line.startsWith(' ')) break

    const referenceFileMatch = line.match(/^\s{4,}-\s*file:\s*(.+)$/)
    if (currentListKey === 'references' && referenceFileMatch) {
      flushReference()
      currentReference = { file: stripYamlScalar(referenceFileMatch[1]) }
      continue
    }

    const referenceLoadWhenMatch = line.match(/^\s{6,}loadWhen:\s*$/)
    if (currentListKey === 'references' && currentReference && referenceLoadWhenMatch) {
      inReferenceLoadWhen = true
      currentReference.loadWhen = {}
      continue
    }

    const referenceConditionMatch = line.match(/^\s{8,}(\w+):\s*(.+)$/)
    if (currentListKey === 'references' && currentReference && inReferenceLoadWhen && referenceConditionMatch) {
      const [, key, rawValue] = referenceConditionMatch
      const value = stripYamlScalar(rawValue)
      if (key === 'task') {
        currentReference.loadWhen = { ...(currentReference.loadWhen ?? {}), task: value }
      } else if (key === 'chapterIndexMax') {
        const numericValue = Number(value)
        if (!Number.isNaN(numericValue)) {
          currentReference.loadWhen = { ...(currentReference.loadWhen ?? {}), chapterIndexMax: numericValue }
        }
      }
      continue
    }
    ...
    if (!match) continue
    if (currentListKey === 'references') flushReference()
    flushList()
  ...
  const manifestKeys = ['category', 'tasks', 'stages', 'triggers', 'priority', 'required', 'references']
  ...
  const manifest: Partial<SkillManifest> = {
    ...
    references: Array.isArray(result.references) ? result.references as SkillManifest['references'] : undefined,
