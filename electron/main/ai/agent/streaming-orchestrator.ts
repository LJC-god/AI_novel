...
export async function runStreamingAgentTask(
  ...
  if (routedTask.task === 'chapter-first-draft') {
  ...
  const maxSteps = resolveStreamingAgentMaxSteps(routedTask.task, optionalSkillDefs.length)
  ...
  const skillUsageHints = routedTask.task === 'chapter-first-draft'
    ? await getRecentSkillUsage(projectId).then(formatSkillUsageHint).catch(() => '')
    : ''

  const enabledModules: string[] = Array.isArray(routedTask.context.enabledContextModules) ? routedTask.context.enabledContextModules : []
  ...
  const chapterToolRules = routedTask.task === 'chapter-assistant' || routedTask.task === 'chapter-first-draft'
    ? [
  ...
  const chapterDraftRules = routedTask.task === 'chapter-first-draft'
    ? [
        '',
        '## 章节初稿 Agent 行为约束',
        '',
        '- 你的主要任务是生成完整章节正文，工具调用只是辅助准备。',
        '- 质量优先：一次只写当前章节正文。禁止并行生成多章、批量输出多章、跨章续写或继续写下一章。',
        '- 即使用户要求一次写多章，也只生成当前章节；本章完成后应先审稿、一致性检查和必要修复，再进入下一章。',
  ...
  const knowledgeTools = createKnowledgeTools({
    ...
    defaultSourceLabel: String(routedTask.context.chapterTitle ?? 'agent')
  })
  ...
  logPrompt('AGENT_STREAM', settings, { system: systemPrompt, user: prompt.user }, routedTask.task, usedSkillIds)
  ...
  try {
    ...
    logResponse('AGENT_STREAM', settings, routedTask.task, loopResult.finalText, Date.now() - requestStartedAt, { usedSkills: usedSkillIds })
    ...
    void recordSkillUsage(projectId, routedTask.task, loopResult.toolCalls).catch(() => {})

    return { result, meta }
  } catch (error) {
    const finishedAt = new Date().toISOString()
    const message = error instanceof Error ? error.message : 'AI Agent 调用失败'
    logError('AGENT_STREAM', settings, routedTask.task, error, Date.now() - requestStartedAt, { usedSkills: usedSkillIds })
    const meta = buildRunMeta(
