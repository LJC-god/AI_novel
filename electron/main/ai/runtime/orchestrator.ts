...
export async function runAiTask(
  ...
  const settingsForRouting = normalizeSettings(resolveSettingsForModelRole(task.settings, task.context.modelRole))
  const routedTask: AiTaskPayload = { ...task, settings: settingsForRouting }
  if (AGENT_TASK_WHITELIST.has(task.task)) {
    if (providerSupportsTools(settingsForRouting)) {
      return runAgentTask(routedTask, knowledgeContext)
    }
...
export async function streamAiTask(
  ...
  const taskHandler = getTaskHandler(routedTask.task)
  const { projectId, skills, usedSkillIds } = await resolveTaskSkills(routedTask)
  logSelection(task.task, skills, knowledgeContext?.usedKnowledge ?? [])
  await enrichTaskContextForGeneration(routedTask, settings)

  const input = buildPromptInput(routedTask, skills, knowledgeContext)
  const prompt = taskHandler.buildPrompt(input)
  const maxTokens = taskHandler.resolveMaxTokens?.(input) ?? resolveMaxTokens(routedTask)
