  ...
  <aside class="ai-panel global-ai-panel">
    ...
    <div ref="conversationRef" class="global-ai-messages messages arc-scrollbar">
      ...
      <template v-else>
        ...
        <div v-if="orchestratorState && orchestratorState.status !== 'applied'" class="global-proposal global-orchestrator">
          <div class="global-proposal__card">
            <div class="global-proposal__header">
              <div>
                <strong>总控阶段确认 · {{ orchestratorState.lastResult.phaseLabel }}</strong>
                <p>{{ orchestratorState.lastResult.confirmationQuestion || orchestratorState.lastResult.recommendedNextAction }}</p>
              </div>
              <div class="global-proposal__header-actions">
                <NButton
                  size="small"
                  secondary
                  :disabled="orchestratorState.status === 'confirmed'"
                  @click="confirmOrchestratorPlan(orchestratorState.selectedInspirationTitle)"
                >
                  确认
                </NButton>
                <NButton
                  size="small"
                  type="primary"
                  :disabled="orchestratorState.status !== 'confirmed' || !hasWritableOrchestratorAssets"
                  @click="applyOrchestratorPlan"
                >
                  写入
                </NButton>
              </div>
            ...
            <div class="global-proposal__sections">
              <section v-if="orchestratorState.lastResult.inspirationDrafts.length" class="global-proposal__section">
                <div class="global-proposal__section-head">
                  <span>灵感选择</span>
                </div>
                <div class="global-orchestrator__choices">
                  <button
                    v-for="item in orchestratorState.lastResult.inspirationDrafts"
                    :key="item.title"
                    type="button"
                    class="global-orchestrator-choice"
                    :class="{ active: orchestratorState.selectedInspirationTitle === item.title }"
                    @click="confirmOrchestratorPlan(item.title)"
                  >
              ...
              <section class="global-proposal__section">
                <div class="global-proposal__section-head">
                  <span>将写入项目</span>
                </div>
                <div class="global-orchestrator__counts">
                  <span>灵感 {{ orchestratorState.lastResult.inspirationDrafts.length }}</span>
                  <span>分卷 {{ orchestratorState.lastResult.volumeDrafts.length }}</span>
                  <span>章纲 {{ orchestratorState.lastResult.chapterOutlineDrafts.length || orchestratorState.lastResult.outlinePlan.length }}</span>
                  <span>世界观 {{ orchestratorState.lastResult.worldviewDrafts.length }}</span>
                  <span>人物 {{ orchestratorState.lastResult.characterDrafts.length }}</span>
                </div>
              </section>
            </div>
          </div>
...
.global-orchestrator .global-proposal__card {
  border-color: color-mix(in srgb, #8b5cf6 28%, var(--arc-border));
  background: color-mix(in srgb, var(--arc-bg-surface) 92%, #8b5cf6);
}

.global-orchestrator__choices {
  display: grid;
  grid-template-columns: 1fr;
  gap: 8px;
}

.global-orchestrator-choice {
  display: flex;
  min-width: 0;
  flex-direction: column;
  align-items: flex-start;
...
.global-orchestrator-choice.active {
  border-color: #8b5cf6;
  box-shadow: 0 0 0 3px color-mix(in srgb, #8b5cf6 14%, transparent);
}

.global-orchestrator-choice strong {
  max-width: 100%;
  overflow: hidden;
  color: var(--arc-text-primary);
  font-size: 12px;
...
.global-orchestrator-choice span {
  color: var(--arc-text-secondary);
  font-size: 11px;
}

.global-orchestrator__counts {
  display: flex;
  flex-wrap: wrap;
  gap: 6px;
}

.global-orchestrator__counts span {
  display: inline-flex;
  align-items: center;
  min-height: 24px;
  padding: 0 9px;
