...
import type { AssistantMode } from '@/composables/useGlobalAssistant'
...
const modeDotClass = (mode: string): string =>
  mode === 'audit' ? 'audit' : mode === 'ingest' ? 'ingest' : mode === 'orchestrate' ? 'orchestrate' : 'correct'
...
function pickMode(mode: AssistantMode): void {
  a.setMode(mode)
      ...
      <div v-else class="ga-thread">
        <div ref="conversationRef" class="ga-thread__scroll arc-scrollbar">
          <div class="ga-thread__inner">
            ...
            <!-- 总控阶段确认卡 -->
            <div v-if="a.orchestratorState.value && a.orchestratorState.value.status !== 'applied'" class="ga-proposal ga-orchestrator">
              <div class="ga-proposal__head">
                <div class="ga-proposal__head-left">
                  <BookMarked :size="17" />
                  <strong>总控阶段确认</strong>
                  <NTag size="small" round :bordered="false" type="info">{{ a.orchestratorState.value.lastResult.phaseLabel }}</NTag>
                </div>
                <div class="ga-proposal__head-actions">
                  <NButton
                    size="small"
                    secondary
                    :disabled="a.orchestratorState.value.status === 'confirmed'"
                    @click="a.confirmOrchestratorPlan(a.orchestratorState.value.selectedInspirationTitle)"
                  >
                    确认当前阶段
                  </NButton>
                  <NButton
                    size="small"
                    type="primary"
                    :disabled="a.orchestratorState.value.status !== 'confirmed' || !a.hasWritableOrchestratorAssets.value"
                    @click="a.applyOrchestratorPlan()"
                  >
                    写入结构资产
                  </NButton>
                </div>
              </div>
              <p class="ga-proposal__summary">{{ a.orchestratorState.value.lastResult.confirmationQuestion || a.orchestratorState.value.lastResult.recommendedNextAction }}</p>

              <div class="ga-proposal__sections">
                <section v-if="a.orchestratorState.value.lastResult.inspirationDrafts.length" class="ga-section">
                  <div class="ga-section__head"><span>灵感选择</span></div>
                  <div class="ga-orchestrator__choices">
                    <button
                      v-for="item in a.orchestratorState.value.lastResult.inspirationDrafts"
                      :key="item.title"
                      type="button"
                      class="ga-orchestrator-choice"
                      :class="{ active: a.orchestratorState.value.selectedInspirationTitle === item.title }"
                      @click="a.confirmOrchestratorPlan(item.title)"
                    >
                ...
                <section class="ga-section">
                  <div class="ga-section__head"><span>将写入项目</span></div>
                  <div class="ga-orchestrator__counts">
                    <span>灵感 {{ a.orchestratorState.value.lastResult.inspirationDrafts.length }}</span>
                    <span>分卷 {{ a.orchestratorState.value.lastResult.volumeDrafts.length }}</span>
                    <span>章纲 {{ a.orchestratorState.value.lastResult.chapterOutlineDrafts.length || a.orchestratorState.value.lastResult.outlinePlan.length }}</span>
                    <span>世界观 {{ a.orchestratorState.value.lastResult.worldviewDrafts.length }}</span>
                    <span>人物 {{ a.orchestratorState.value.lastResult.characterDrafts.length }}</span>
                  </div>
                </section>
              </div>
            </div>
...
.ga-dot.orchestrate { background: #8b5cf6; }
...
.ga-orchestrator {
  border-color: color-mix(in srgb, #8b5cf6 26%, var(--arc-border, #e5e7eb));
  background: color-mix(in srgb, var(--arc-bg-surface, #ffffff) 92%, #8b5cf6);
}

.ga-orchestrator .ga-proposal__head svg {
  color: #8b5cf6;
}

.ga-orchestrator__choices {
  display: grid;
  grid-template-columns: repeat(auto-fit, minmax(160px, 1fr));
  gap: 8px;
}

.ga-orchestrator-choice {
  display: flex;
  min-width: 0;
  flex-direction: column;
  align-items: flex-start;
...
.ga-orchestrator-choice.active {
  border-color: #8b5cf6;
  box-shadow: 0 0 0 3px color-mix(in srgb, #8b5cf6 14%, transparent);
}

.ga-orchestrator-choice strong {
  max-width: 100%;
  overflow: hidden;
  text-overflow: ellipsis;
  white-space: nowrap;
...
.ga-orchestrator-choice span {
  color: var(--arc-text-secondary, #4b5563);
  font-size: 11.5px;
}

.ga-orchestrator__counts {
  display: flex;
  flex-wrap: wrap;
  gap: 8px;
}

.ga-orchestrator__counts span {
  display: inline-flex;
  align-items: center;
  min-height: 26px;
  padding: 0 10px;
